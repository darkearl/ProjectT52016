...
function fn_get_users($params, &$auth, $items_per_page = 0, $custom_view = '')
{
    ...
    if (isset($params['phone']) && fn_string_not_empty($params['phone'])) {
        $condition['phone'] = db_quote(" AND (?:users.phone LIKE ?l OR ?:user_profiles.b_phone LIKE ?l OR ?:user_profiles.s_phone LIKE ?l)",
                                      "%".trim($params['phone'])."%","%".trim($params['phone'])."%","%".trim($params['phone'])."%");
    }
