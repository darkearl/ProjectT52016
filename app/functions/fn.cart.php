...
function fn_finish_payment($order_id, $pp_response, $force_notification = array())
{
    ...
    if (!empty($valid_id)) {
        ...
        if ($pp_response['order_status'] == 'N' && !empty(Tygh::$app['session']['cart']['placement_action']) && Tygh::$app['session']['cart']['placement_action'] == 'repay') {
...
function fn_delete_order($order_id)
{
    ...
    if (!empty($shipment_ids)) {
        db_query('DELETE FROM ?:shipments WHERE shipment_id IN (?n)', $shipment_ids);
...
function fn_get_customer_location($auth, $cart, $billing = false)
{
    ...
    if (empty($s_info['country'])) {
        $s_info['country'] = Registry::get('settings.General.default_country');
    }

    if (empty($s_info['state'])) {
        $s_info['state'] = Registry::get('settings.General.default_state');
    }

    if (empty($s_info['city'])) {
        $s_info['city'] = Registry::get('settings.General.default_city');
    }
...
function fn_get_orders($params, $items_per_page = 0, $get_totals = false, $lang_code = CART_LANGUAGE)
{
    ...
    if (isset($params['issuer']) && fn_string_not_empty($params['issuer'])) {
        ...
        $_condition .= db_quote(" $union_condition ?:orders.issuer_id IN (?n)", $issuer_ids);
    }
