...
function fn_image_verification($condition, $req)
{
    ...
    if (Registry::get('settings.Image_verification.hide_after_validation') == 'Y') {
        Tygh::$app['session']['image_verification_ok'] = true;
    }
...
function fn_needs_image_verification($condition)
{
    ...
    return Registry::get('settings.Image_verification.use_for.' . $condition) == 'Y'
        && empty(Tygh::$app['session']['image_verification_ok']);
}
...
function fn_check_permission_manage_own_profile()
{
    if (Registry::get('runtime.controller') == 'profiles' && Registry::get('runtime.mode') == 'update') {
        return (empty($_REQUEST['user_id']) || $_REQUEST['user_id'] == Tygh::$app['session']['auth']['user_id']) ? true : false;
...
function fn_update_status($status, $status_data, $type, $lang_code = DESCR_SL)
{
    ...
    // Create status to have its identifer
...
function fn_get_logo_image_path($logo_image, $type, $style_id)
{
    if (is_file(realpath($logo_image))) {
        ...
        if (is_file($logos_path . $style_id . '/' . $logo_image)) {
            $logo_filepath = $logos_path . $style_id . '/' . $logo_image;
        } elseif (is_file($logos_path . $logo_image)) {
            $logo_filepath = $logos_path . $logo_image;
        } else {
            $manifest = Themes::factory(fn_get_theme_path('[theme]', 'C'))->getManifest();
            $logo_filepath = '';

            if ($type == 'favicon') {
                if (!empty($manifest['favicon'])) {
                    $logo_filepath = $manifest['favicon'];
                }
            } elseif ($type == 'mail') {
                if (!empty($manifest['mail'])) {
                    $logo_filepath = $manifest['mail'];
                }
            } elseif (!empty($manifest['logo'])) {
                $logo_filepath = $manifest['logo'];
            } elseif (!empty($manifest['theme'])) {
                // Backward compatibility
                $logo_filepath = $manifest['theme'];
            }

            if (!empty($logo_filepath)) {
                $logo_filepath = fn_get_theme_path("[themes]/[theme]/{$logo_filepath}", 'C');
                if (!is_file($logo_filepath)) {
                    $logo_filepath = '';
                }
            }
        }
