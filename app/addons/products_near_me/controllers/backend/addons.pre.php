...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'update' && $_REQUEST['addon'] == 'products_near_me' && !empty($_REQUEST['sl_settings'])) {
        $sl_settings = $_REQUEST['sl_settings'];

        fn_update_products_near_me_settings($sl_settings);
...
if ($mode == 'update') {
    ...
    if ($_REQUEST['addon'] == 'products_near_me') {
        Tygh::$app['view']->assign('sl_provider_templates', fn_get_products_near_me_map_provider_setting_templates());
        Tygh::$app['view']->assign('sl_settings', fn_get_products_near_me_settings());
    }

}

function fn_get_products_near_me_map_provider_setting_templates()
{
    $templates = array();

    $skin_path = fn_get_theme_path('[themes]/[theme]', 'A');
    $relative_directory_path = 'addons/products_near_me/settings/';
...
function fn_update_products_near_me_settings($sl_settings, $company_id = null)
{
    if (!$setting_id = Settings::instance()->getId('products_near_me_', '')) {
        $setting_id = Settings::instance()->update(array(
            'name' =>           'products_near_me_',
            'section_id' =>     0,
            'section_tab_id' => 0,
