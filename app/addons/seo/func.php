...
function fn_seo_update_tree_object($object_id, $object_type, $params)
{
    ...
    $regenerate_children && fn_iterate_through_seo_names(
        'fn_regenerate_seo_name',
        db_quote("path = ?s AND type IN (?a) ?p", $params['old_id_path'], $params['object_types'], $condition)
    );

    // Regenerate grandchildren SEO-names
    $regenerate_grandchildren && fn_iterate_through_seo_names(
        'fn_regenerate_seo_name',
        db_quote("path LIKE ?l AND type IN (?a) ?p", $params['old_id_path'] . '/%', $params['object_types'], $condition)
    );
...
function fn_seo_delete_category_after(&$category_id)
{
    ...
    fn_seo_update_tree_object($category_id, 'c', array(
        'company_id' => $old_category_data['company_id'],
        'old_id_path' => $old_category_data['id_path'],
        'object_types' => array('c', 'p'),
        'after_deletion' => true,

        // No need to regenerate grandchildren SEO-names, because directory deletion is recursive itself,
        // i.e this hook call will be applied to every subcategory of category that is being deleted.
        'regenerate_grandchildren' => false,
    ));
...
/**
 * Adds canonical url, prev and next meta-links
 */
