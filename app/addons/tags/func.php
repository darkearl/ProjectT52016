...
/**
 * Builds JOIN expression that will narrow query results
 * down to active objects only.
 *
 * @param $params Parameter list
 * @return string JOIN expression to be appended to query or
 *                 false in the case of unknown object type.
 */
function fn_tags_build_enabled_products_join($params)
{
    $selects = array(
        'A' => "SELECT cscart_pages.page_id as obj_id, 'A' as object_type from cscart_pages WHERE cscart_pages.status = 'A'",
        'P' => "SELECT cscart_products.product_id as obj_id, 'P' as object_type from cscart_products WHERE cscart_products.status = 'A'",
    );

    $join_expression = "INNER JOIN ((";
    $on_expression = ")) statuses ON statuses.obj_id = cscart_tag_links.object_id AND statuses.object_type = cscart_tag_links.object_type";
