...
class App
{
    ...
    protected function processThemesFiles($schema)
    {
        ...
        $deleted_files = $changed_files = array();
        ...
        foreach ($schema['files'] as $file_path => $file_data) {
            if (strpos($file_path, $repo_path) !== false) {
                ...
                if ($file_data['status'] === 'deleted') {
                    $deleted_files[$theme_name][] = $path;
                } else {
                    $changed_files[$theme_name][] = $path;
                }
            }
        }
        ...
        foreach ($themes as $theme_name) {
            ...
            if (!empty($changed_files[$parent_theme])) {
                foreach ($changed_files[$parent_theme] as $file_path) {
            ...
            if (!empty($deleted_files[$parent_theme])) {
                foreach ($deleted_files[$parent_theme] as $file_path) {
                    fn_rm($this->config['dir']['root'] . '/design/themes/' . $theme_name . '/' . $file_path);
                }
            }
        }

        return array_merge_recursive($changed_files, $deleted_files);
    }
