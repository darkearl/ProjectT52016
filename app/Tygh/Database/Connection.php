...
class Connection
{
    ...
    {
        if ((!empty($result) || !$this->errorCode())) {
            // it's ok
        } else {

            $error = array (
                'message' => $this->db->error() . ' <b>(' . $this->db->errorCode() . ')</b>',
                'query' => $query,
            );
    ...
    {
        if (in_array($this->errorCode(), $this->lost_connection_codes) && $this->reconnects < $this->max_reconnects) {
            $this->db->disconnect();
            $this->reconnects++;

            $dbc_data = $this->dbs[$this->dbc_name];

            $this->connect($dbc_data['user'], $dbc_data['passwd'], $dbc_data['host'], $dbc_data['database'], $dbc_data['params']);
